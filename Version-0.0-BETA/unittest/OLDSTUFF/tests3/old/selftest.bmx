...
' Get grammar defintion from parser
Local grammar1:TGrammar = PEG.grammar
...
Local tree1:TParseTree = PEG.Parse( peg1 )
...
DebugStop
Local tree2:TParseTree = PEG.Parse( peg2 )
Local grammar2:TGrammar = PEG.grammar
...
' Convert PEG grammar as JSON
Local json1:String = PEG.toJSON()


' Load JSON as a new Parser
Local PEG2parser:TPackrat_Parser = New TPackrat_Parser()
PEG2parser.fromJSON( json1 )

' Convert PEG2 into JSON
Local json2:String = PEG2parser.toJSON()
...
If json1.length <> json2.length
	success = False
	Print "## FAIL - File length is different"
Else
	Local line:Int = 1
	Local col:Int = 1
	For Local n:Int = 0 Until json1.length
		Local ch1:Int = Asc( json1[n] )
		Local ch2:Int = Asc( json2[n] )
		If ch1<>ch2
			Print "## FAIL - File is different at line "+ line + ", col " + col
			success = False
			Exit
		End If
		If ch1=13
